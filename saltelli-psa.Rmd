...
TornadoOpt(as.matrix(params), as.matrix(cbind(results$nmb100s2, results$nmb100s3)), opt=1, title="Sensitivity Scenario 2 WIP 100k")
...
## Percent of Runs Greater than Zero
...
* `r percent("nmb100s2")` of scenario 2 net monetary benefits are greater than zero at a williness to pay threshold of $100k.
...
* `r percent("nmb50s2")` of scenario 2 net monetary benefits are greater than zero at a williness to pay threshold of $50k.
...
plot(results$dQALY2 - results$dQALY0, 
     results$dCOST2 - results$dCOST0,
     main="Scenario 2", sub="Cost Effectiveness compared to reference",
     xlab="Delta dQALY",
     ylab="Delta dCOST",
     col=rgb(0,0,0,0.25),
     pch=16)
abline(a=0, b=100000, col='red')
abline(a=0, b=50000, col='blue')
legend(-0.0155, -1875, c("100k", "50k"), col=c("red", "blue"), lty=1)
...
## Saltelli Method Results 100k WTP Scenario 2

```{r, results='asis'}
single <- read.csv("saltelli-single-2.txt", header=FALSE)
total  <- read.csv("saltelli-total-2.txt",  header=FALSE)

saltelli <- data.frame(Variable = ranges$Variable,
                       Single   = unname(single),
                       Total    = unname(total))

tangram(saltelli[order(saltelli$Single, decreasing=TRUE),c("Variable", "Single")],
  as.character=TRUE,
  id="Single 2",
  caption="Saltelli Ranking for Direct Contribution to Variance",
  pct_width=0.8,
  footnote="Scaled to 100%",
  style="nejm") %>% del_col(1)


tangram(saltelli[order(saltelli$Total, decreasing=TRUE),c("Variable", "Total")],
  as.character=TRUE,
  id="Total 2",
  caption="Saltelli Ranking Including All Interactions to Variance",
  footnote="Total exceeds 100% since variance contribution occurs in multiple terms via interaction",
  pct_width=0.8,
  style="nejm") %>% del_col(1)
```
...
## Saltelli Method Results 100k WTP Scenario 3

```{r, results='asis'}
single <- read.csv("saltelli-single-3.txt", header=FALSE)
total  <- read.csv("saltelli-total-3.txt",  header=FALSE)
...
tangram(saltelli[order(saltelli$Total, decreasing=TRUE),c("Variable", "Total")],
  ...
  id="Total 3",
  caption="Saltelli Ranking Including All Interactions to Variance",
  footnote="Total exceeds 100% since variance contribution occurs in multiple terms via interaction 3",
  pct_width=0.8,
  style="nejm") %>% del_col(1)
```